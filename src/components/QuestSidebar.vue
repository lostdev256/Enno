...
function onCtxAction(action: string) {
  switch (action) {
    case 'delete-quest': if (ctxTargetId.value) emit('delete-quest', ctxTargetId.value); break
    ...
    case 'add-quest': emit('create-quest'); break
        ...
        <template #item="{ element: group }">
          <div class="group">
            ...
            <div v-show="group.expanded" class="group-content">
              <draggable :list="group.quests" group="quests" item-key="id" :animation="200" ghost-class="drag-ghost" chosen-class="drag-chosen" @end="onDragEnd">
                <template #item="{ element }">
                  <div class="card-item" :class="{ selected: selectedId === element.id }" @click="emit('select', element.id)" @contextmenu.stop="onContextMenu($event, 'quest', element.id)">
...
.card-avatar { width:28px; height:28px; border-radius:6px; object-fit:cover; flex-shrink:0; background:#2a2a3e; }

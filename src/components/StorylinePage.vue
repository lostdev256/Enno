...
async function createScene() {
    const r = await window.ennoAPI.createScene();
    if (r) await loadList();
}
...
async function renameGroup(id: string) {
    ...
    if (!n) return;
    await window.ennoAPI.renameSceneGroup(id, n);
    await loadList();
}

async function handleReorder(data: { groups: SidebarGroup[], ungrouped: SidebarScene[] }) {
    await window.ennoAPI.reorderScenes({
...
async function loadStoryline() {
    ...
    if (data) {
        ...
        for (const gp of data.groupPositions) framePositions.value.set(gp.groupId, {
            x: gp.x,
            y: gp.y,
            width: gp.width,
            height: gp.height
...
function onBoardMouseDown(e: MouseEvent) {
    ...
    if (e.button === 1 || (e.button === 0 && !target.closest(".storyline-node, .group-frame-header, .resize-handle, .snode-pin"))) {
        ...
        panSP = {x: e.clientX, y: e.clientY};
        panST = {x: transform.value.x, y: transform.value.y};
...
function onPanMove(e: MouseEvent) {
    ...
    transform.value.x = panST.x + (e.clientX - panSP.x);
    transform.value.y = panST.y + (e.clientY - panSP.y);
}
...
function onWheel(e: WheelEvent) {
    ...
    let ns = transform.value.scale * (1 + d * 0.05);
...
function startFrameDrag(e: MouseEvent, groupId: string) {
    ...
    window.addEventListener("mousemove", onFrameDrag);
...
async function stopFrameDrag() {
    if (dragFrame) {
        const fp = framePositions.value.get(dragFrame.groupId)!;
        await window.ennoAPI.updateStorylineGroupPosition(dragFrame.groupId, fp.x, fp.y, fp.width, fp.height);
    }
    dragFrame = null;
    window.removeEventListener("mousemove", onFrameDrag);
...
function startFrameResize(e: MouseEvent, groupId: string) {
    ...
    window.addEventListener("mousemove", onFrameResize);
...
async function stopFrameResize() {
    if (resizeFrame) {
        const fp = framePositions.value.get(resizeFrame.groupId)!;
        await window.ennoAPI.updateStorylineGroupPosition(resizeFrame.groupId, fp.x, fp.y, fp.width, fp.height);
    }
    resizeFrame = null;
    window.removeEventListener("mousemove", onFrameResize);
...
async function stopNodeDrag() {
    if (dragNode) {
        const pos = nodePositions.value.get(dragNode.id)!;
        await window.ennoAPI.updateStorylineNode(dragNode.id, pos.x, pos.y);
    }
    dragNode = null;
...
function startDrawConn(e: MouseEvent, nodeId: string, pin: string) {
    ...
    e.stopPropagation();
    drawing.value = true;
    drawSourceId.value = nodeId;
    drawSourcePin.value = pin;
    mousePos.value = clientToCanvas(e.clientX, e.clientY);
    window.addEventListener("mousemove", onDrawMove);
    window.addEventListener("mouseup", stopDrawConn);
}
...
async function stopDrawConn(e: MouseEvent) {
    ...
    window.removeEventListener("mouseup", stopDrawConn);
...
async function onDrop(e: DragEvent) {
    ...
    const scene = [...groups.value.flatMap(g => g.scenes.map(s => ({
        ...s,
        groupId: g.id
    }))), ...ungrouped.value.map(s => ({...s, groupId: null as string | null}))].find(s => s.id === sceneId);
...
async function onCtxAction(action: string) {
    if (action === "add:global_action") {
        ...
        await window.ennoAPI.deleteStorylineNode(ctxNodeId);
        await loadStoryline();
    } else if (action === "delete-conn" && ctxConnId) {
        await window.ennoAPI.deleteStorylineConnection(ctxConnId);
        await loadStoryline();
    }
}
...
function getSceneName(refId: string | null): string {
    ...
    for (const g of groups.value) {
        const s = g.scenes.find(sc => sc.id === refId);
        if (s) return s.name;
    }
...
function getNodeExitPins(node: any): { id: string; label: string }[] {
    if (node.nodeType === "scene") {
        ...
        for (const g of groups.value) {
            ...
            if (s && s.exitPins) {
                try {
                    const pins = JSON.parse(s.exitPins);
        ...
        if (us && us.exitPins) {
            try {
                const pins = JSON.parse(us.exitPins);
                if (pins.length) return pins;
            } catch {
            }
        }
...
function startEditNode(node: any) {
    ...
    try {
        editDesc.value = JSON.parse(node.data).description || "";
    } catch {
        editDesc.value = "";
    }
}
...
onMounted(async () => {
    ...
    window.ennoAPI.onProjectStateChange(async (state) => {
        if (state.isOpen) {
            ...
            groups.value = [];
            ungrouped.value = [];
            storylineData.value = {nodes: [], connections: [], groupPositions: []};
            nodePositions.value = new Map();
            framePositions.value = new Map();
            pinPositions.value = new Map();
        }
    });
});
</script>
...
<template>
    <div class="storyline-page">
        ...
        <div class="board-area" ref="boardRef" @mousedown="onBoardMouseDown" @wheel.prevent="onWheel"
             @contextmenu="onBoardContextMenu" @drop="onDrop" @dragover.prevent>
            <div class="board-canvas"
                ...
                <svg class="conn-layer">
                    <g v-for="c in connectionPaths" :key="c.id">
                        <path :d="c.d" stroke="transparent" stroke-width="12" fill="none" class="conn-hit"
                              @contextmenu="onConnContextMenu($event, c.id)"/>
                    ...
                    >
                        ...
                        <div class="snode-body">
                            <div class="snode-header"
                                 :class="{ scene: node.nodeType === 'scene', global: node.nodeType === 'global_action' }"
                                 @mousedown="startNodeDrag($event, node.id)" @dblclick.stop="openScene(node)">
                                {{ node.nodeType === "scene" ? "🎬" : "📝" }} {{ getSceneName(node.refId) }}
                            </div>
                            <template v-if="node.nodeType === 'global_action'">
                                <div class="global-content">
                                    <div v-if="editingNodeId !== node.id" class="block-text editable"
                                         @dblclick.stop="startEditNode(node)">
                                        {{
                                            (function () {
                                                try {
                                                    return JSON.parse(node.data).description || t("scenes.dblclickToEdit");
                                                } catch {
                                                    return "";
                                                }
                                            })()
                                    ...
                                    <textarea v-else class="block-input" v-model="editDesc" @blur="saveEditNode(node)"
                                              @keydown.escape="cancelEditNode" @keydown.ctrl.enter="saveEditNode(node)"
                                              rows="2" @click.stop></textarea>

                                    <div class="choices-area"
                                         v-if="(function(){ try{ return JSON.parse(node.data).choices?.length } catch{ return false } })()">
                                        <div
                                            v-for="ch in (function(){ try{ return JSON.parse(node.data).choices || [] } catch{ return [] } })()"
                                            :key="ch.id" class="choice-row-inline">
                                            <input class="choice-input" :value="ch.label"
                                                   @change="updateGlobalChoice(node, ch.id, ($event.target as HTMLInputElement).value)"
                                                   @click.stop/>
                                            <button class="icon-btn red" @click.stop="removeGlobalChoice(node, ch.id)">
                                                ✕
                                            </button>
                                        </div>
                                    </div>
                                    <button class="add-choice-btn" @click.stop="addGlobalChoice(node)">+
                                        {{ t("scenes.addChoice") }}
                                    </button>
                                </div>
                            </template>
...
.resize-handle {
    ...
    cursor: nwse-resize;
    pointer-events: auto;
    background: linear-gradient(135deg, transparent 50%, rgba(100, 108, 255, 0.3) 50%);

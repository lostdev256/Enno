...
const renderedLinks = computed(() => {
    ...
    let settings: any = {};
    try {
        settings = JSON.parse(props.currentMode.settings);
    } catch {
    }
    ...
    <div class="board-container" ref="boardRef"
        ...
        <div class="board-canvas" :style="canvasStyle">
            ...
            <svg class="links-layer">
                <defs>
                    ...
                    <template v-for="link in renderedLinks" :key="link.id">
                        <marker :id="`arrowhead-${link.id}`" markerWidth="8" markerHeight="6" refX="28" refY="3"
                                orient="auto">
                            <polygon points="0 0, 8 3, 0 6" :fill="link.color"/>
                        </marker>
                        <marker :id="`arrowhead-start-${link.id}`" markerWidth="8" markerHeight="6" refX="-20" refY="3"
                                orient="auto">
            ...
            <div class="nodes-layer">
                ...
                >
                    <img :src="getAvatarSrc(getChar(node.characterId)?.avatarUrl)" class="node-avatar"
                         draggable="false"/>

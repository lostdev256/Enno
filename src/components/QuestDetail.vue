    ...
    <div class="detail" v-if="quest">
        ...
        <div class="detail-section">
            <RichTextEditor
                ...
                :placeholder="t('quests.writeDescription')"
                :emptyText="t('quests.noDescription')"
            />
        </div>
        ...
        <div class="detail-section">
            <div class="section-header">
                ...
                <button class="edit-btn" @click="emit('add-gallery', quest.id)" :title="t('quests.addImages')">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            ...
            <div class="gallery-strip" v-if="quest.gallery.length > 0">
                <div v-for="(img, i) in quest.gallery" :key="img.id" class="gallery-thumb" @click="openLightbox(i)"
                     @contextmenu="onGalleryContextMenu($event, img.id)">
                    <img :src="'enno://' + img.path" :alt="`Gallery ${i + 1}`"/>
                </div>
                <button class="gallery-add-btn" @click="emit('add-gallery', quest.id)" :title="t('quests.addMore')">
            ...
            <div v-else class="gallery-empty">
                <button class="gallery-empty-btn" @click="emit('add-gallery', quest.id)">
                    ...
                    <span>{{ t("quests.addImagesToGallery") }}</span>
                </button>
            </div>
        </div>
        ...
        <ImageLightbox :images="quest.gallery.map(g => g.path)" :start-index="lightboxStartIndex"
    ...
    <div v-else class="detail-empty">
        <div class="empty-icon">
            <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                ...
                <path d="M6 18h36M18 10v8" stroke="currentColor" stroke-width="1.5"/>
                <circle cx="24" cy="30" r="4" stroke="currentColor" stroke-width="1.5"/>
            </svg>
        </div>
        <p class="empty-text">{{ t("quests.selectToView") }}</p>
    </div>
</template>

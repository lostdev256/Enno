...
function onContextMenu(e: MouseEvent, type: "scene" | "group" | "empty", id?: string) {
    ...
    if (type === "scene") {
        ctxItems.value = [{label: t("scenes.deleteScene"), action: "delete-scene", icon: "🗑"}];
    } else if (type === "group") {
        ctxItems.value = [
        ...
        ctxItems.value = [
            {label: t("scenes.addScene"), action: "add-scene", icon: "➕"},
...
function onCtxAction(action: string) {
    switch (action) {
        case "delete-scene":
            if (ctxTargetId.value) emit("delete-scene", ctxTargetId.value);
            break;
        case "rename-group":
...
<template>
    <aside class="sidebar" @contextmenu="onContextMenu($event, 'empty')" @click="onDocClick">
        <div class="sidebar-toolbar">
            <div class="add-dropdown-wrap">
                ...
                <Transition name="dropdown">
                    <div v-if="showAddDropdown" class="add-dropdown">
                        <button class="add-dropdown-item" @click="addScene"><span>🎬</span> {{ t("scenes.addScene") }}
                        </button>
            ...
            <button class="tb-btn" @click="selectedId && emit('delete-scene', selectedId)" :disabled="!selectedId"
        ...
        <div class="sidebar-list">
                ...
                <template #item="{ element: group }">
                    <div class="group">
                        ...
                        <div v-show="group.expanded" class="group-content">
                            <draggable :list="group.scenes" group="scenes" item-key="id" :animation="200"
                                       ghost-class="drag-ghost" chosen-class="drag-chosen" @end="onDragEnd">
            ...
            <div v-if="ungrouped.length > 0" class="ungrouped-section">
                ...
                <draggable :list="ungrouped" group="scenes" item-key="id" :animation="200" ghost-class="drag-ghost"

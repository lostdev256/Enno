      ...
      <div class="add-dropdown-wrap">
        ...
        <Transition name="dropdown">
          <div v-if="showAddDropdown" class="add-dropdown">
            <button class="add-dropdown-item" @click="addScene"><span>🎬</span> {{ t('scenes.addScene') }}</button>
            <button class="add-dropdown-item" @click="addGroup"><span>📁</span> {{ t('scenes.addGroup') }}</button>
          </div>
        </Transition>
      </div>
      <button class="tb-btn" @click="selectedId && emit('delete-scene', selectedId)" :disabled="!selectedId" :title="t('scenes.deleteScene')">

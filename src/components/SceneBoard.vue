...
function cancelEdit() { editingActionId.value = null }
...
<template>
  <div class="scene-board" ref="boardRef" @mousedown="onBoardMouseDown" @wheel.prevent="onWheel" @contextmenu="onBoardContextMenu">
    <!-- Scene title bar -->
    ...
    <div class="board-canvas" :style="canvasStyle">
      ...
      <svg class="connections-layer">
        <g v-for="conn in connectionPaths" :key="conn.id">
          <path :d="conn.d" stroke="transparent" stroke-width="12" fill="none" class="conn-hitbox" @contextmenu="onConnectionContextMenu($event, conn.id)" />
          <path :d="conn.d" stroke="#a5b4fc" stroke-width="2" fill="none" class="conn-path" style="pointer-events:none;" />
        </g>
        ...
        <div class="block-body">
          ...
          <div class="pins-right">
            ...
            <template v-if="action.actionType === 'exit'">
              <div v-for="pin in (parseData(action.data).pins || [])" :key="pin.id" class="pin-wrap" @mousedown="startDrawing($event, action.id, pin.id)">
                <div class="pin-dot output" :data-pin-key="`${action.id}:${pin.id}:out`" :title="pin.label"></div>
              </div>
            </template>
            <!-- For ENTRY / plain actions (no choices): single out pin -->
            <template v-else-if="action.actionType === 'entry' || !(parseData(action.data).choices || []).length">
...
<style scoped>
.scene-board { position:relative; width:100%; height:100%; background:#1a1a20; overflow:hidden; cursor:grab; }
.scene-board:active { cursor:grabbing; }

.scene-title-bar { position:absolute; top:0; left:0; right:0; padding:8px 16px; background:rgba(0,0,0,0.4); backdrop-filter:blur(10px); z-index:20; display:flex; align-items:center; gap:8px; border-bottom:1px solid rgba(255,255,255,0.06); }
.scene-title { font-size:14px; font-weight:600; color:#e0e0f0; display:flex; align-items:center; gap:8px; }
.title-edit-btn { background:none; border:none; cursor:pointer; font-size:12px; opacity:0.5; transition:opacity .2s; }
.title-edit-btn:hover { opacity:1; }
.scene-name-input { background:rgba(255,255,255,0.06); border:1px solid rgba(100,108,255,0.4); border-radius:6px; padding:4px 8px; color:#e0e0f0; font-size:14px; font-weight:600; font-family:inherit; outline:none; }

/* Characters panel */
.char-panel { position:absolute; top:48px; left:12px; background:rgba(30,30,38,0.9); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.08); border-radius:10px; z-index:15; min-width:180px; max-width:240px; box-shadow:0 8px 32px rgba(0,0,0,0.4); }
.char-panel-header { display:flex; align-items:center; justify-content:space-between; padding:8px 12px; border-bottom:1px solid rgba(255,255,255,0.06); font-size:11px; font-weight:600; color:#8888a0; text-transform:uppercase; letter-spacing:.5px; }
.char-panel-btn { background:none; border:none; color:#818cf8; cursor:pointer; font-size:16px; padding:2px; transition:color .2s; }
.char-panel-btn:hover { color:#c7d2fe; }
.char-panel-list { padding:6px; max-height:200px; overflow-y:auto; }
.char-panel-item { display:flex; align-items:center; gap:8px; padding:4px 6px; border-radius:6px; transition:background .12s; }
.char-panel-item:hover { background:rgba(255,255,255,0.04); }
.char-panel-avatar { width:24px; height:24px; border-radius:5px; object-fit:cover; background:#2a2a3e; }
.char-panel-name { font-size:12px; color:#c0c0d8; flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.char-panel-remove { background:none; border:none; color:#666; cursor:pointer; font-size:10px; padding:2px; opacity:0; transition:opacity .15s; }
.char-panel-item:hover .char-panel-remove { opacity:1; }
.char-panel-remove:hover { color:#ef4444; }
.char-panel-empty { font-size:11px; color:#555; padding:8px; text-align:center; }

/* Picker dialog */
.picker-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.6); z-index:9999; display:flex; align-items:center; justify-content:center; }
.picker-dialog { background:#2c2c34; border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:20px; min-width:300px; max-width:400px; max-height:60vh; display:flex; flex-direction:column; }
.picker-dialog h3 { margin:0 0 12px; font-size:16px; color:#e0e0f0; }
.picker-list { flex:1; overflow-y:auto; display:flex; flex-direction:column; gap:4px; }
.picker-item { display:flex; align-items:center; gap:10px; padding:8px 12px; border-radius:8px; cursor:pointer; transition:background .12s; }
.picker-item:hover { background:rgba(100,108,255,0.15); }
.picker-avatar { width:32px; height:32px; border-radius:6px; object-fit:cover; background:#2a2a3e; }
.picker-item span { font-size:14px; color:#d0d0e0; }
.picker-cancel { margin-top:12px; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.1); border-radius:8px; padding:8px; color:#999; cursor:pointer; font-family:inherit; transition:all .15s; }
.picker-cancel:hover { background:rgba(255,255,255,0.1); color:#ccc; }
...
.connections-layer { position:absolute; top:0; left:0; width:1px; height:1px; overflow:visible; pointer-events:none; z-index:1; }
.conn-hitbox { pointer-events:stroke; cursor:pointer; }
.conn-hitbox:hover + .conn-path { stroke-width:3; stroke:#c7d2fe; }

/* Action blocks */
.action-block { position:absolute; pointer-events:auto; min-width:200px; max-width:320px; background:#252530; border:1px solid rgba(255,255,255,0.08); border-radius:8px; box-shadow:0 4px 16px rgba(0,0,0,0.4); user-select:none; z-index:2; }
.block-header { padding:6px 12px; border-radius:7px 7px 0 0; display:flex; align-items:center; gap:8px; cursor:grab; }
.block-header:active { cursor:grabbing; }
.block-title { font-size:11px; font-weight:700; color:#fff; text-transform:uppercase; letter-spacing:.5px; }
.block-char-name { font-size:11px; color:rgba(255,255,255,0.7); font-weight:400; }
/* Block auto-height */
.block-body { display:flex; align-items:flex-start; padding:8px 0; }
.block-content { flex:1; padding:0 8px; font-size:12px; color:#aaa; line-height:1.5; word-break:break-word; }
.block-text { white-space:pre-wrap; word-break:break-word; }

/* Pin layout */
.pins-left, .pins-right { display:flex; flex-direction:column; justify-content:flex-start; padding:4px 0; gap:0; }
.pins-left { padding-left:0; width:14px; align-items:center; }
.pins-right { padding-right:0; width:14px; align-items:center; }
.pin-wrap { display:flex; align-items:center; justify-content:center; height:28px; cursor:crosshair; }
.pin-dot { width:12px; height:12px; border-radius:50%; border:2px solid rgba(255,255,255,0.3); background:#1a1a20; transition:all .15s; flex-shrink:0; cursor:crosshair; }
.pin-dot.input { border-color:#a5b4fc; }
.pin-dot.output { border-color:#f59e0b; }
.pin-dot:hover { transform:scale(1.3); background:rgba(255,255,255,0.2); }
.pin-dot.choice-pin { margin-left:4px; cursor:crosshair; }

/* Editing */
.editable { cursor:text; border:1px solid transparent; border-radius:4px; padding:2px 4px; margin:-2px -4px; transition:border-color .15s; white-space:pre-wrap; }
.editable:hover { border-color:rgba(255,255,255,0.1); }
.block-input { width:100%; background:rgba(255,255,255,0.06); border:1px solid rgba(100,108,255,0.4); border-radius:4px; padding:4px 6px; color:#e0e0f0; font-size:12px; font-family:inherit; outline:none; resize:none; box-sizing:border-box; }
.block-input:focus { border-color:#818cf8; }

/* Quest link */
.quest-link { margin-top:6px; padding:3px 6px; background:rgba(245,158,11,0.1); border:1px solid rgba(245,158,11,0.2); border-radius:4px; font-size:11px; color:#f59e0b; cursor:pointer; transition:background .15s; }
.quest-link:hover { background:rgba(245,158,11,0.2); }
...
.choice-input { flex:1; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:4px; padding:2px 6px; color:#ccc; font-size:11px; font-family:inherit; outline:none; min-width:0; }
.choice-input:focus { border-color:rgba(100,108,255,0.4); }
.add-choice-btn { background:none; border:none; color:#666; cursor:pointer; font-size:11px; padding:3px 0; font-family:inherit; transition:color .15s; display:block; }
.add-choice-btn:hover { color:#818cf8; }

/* Exit pins */
.exit-pins-list { display:flex; flex-direction:column; gap:4px; }
.exit-row { display:flex; align-items:center; gap:4px; }
.exit-pin-input { flex:1; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:4px; padding:2px 6px; color:#ccc; font-size:11px; font-family:inherit; outline:none; }
.exit-pin-input:focus { border-color:rgba(239,68,68,0.4); }
